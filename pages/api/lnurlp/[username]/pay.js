...
import { lnurlPayMetadata, lnurlpVerifyUrl } from '@/lib/lnurl'
import { schnorr } from '@noble/curves/secp256k1'
import { createHash } from 'crypto'
import { LNURLP_COMMENT_MAX_LENGTH, PROXY_PAYER_MIN_MSATS, MAX_WALLET_INVOICE_SATS } from '@/lib/constants'
import { formatMsats, toPositiveBigInt } from '@/lib/format'
import assertGofacYourself from '@/api/resolvers/ofac'
import { validateSchema, lud18PayerDataSchema, characterLength } from '@/lib/validate'
...
export default async ({ query: { username, amount, nostr, comment, payerdata: payerData }, headers }, res) => {
  ...
  if (amountMsats > MAX_WALLET_INVOICE_SATS * 1000) {
    return res.status(400).json({ status: 'ERROR', reason: `amount must be <= ${MAX_WALLET_INVOICE_SATS * 1000} msats` })
  }
  ...
  try {
    ...
    if (nostr) {
      ...
      try {
        lud18Data = JSON.parse(payerData)
      } catch (err) {
      ...
      try {
        await validateSchema(lud18PayerDataSchema, lud18Data)
      } catch (err) {
