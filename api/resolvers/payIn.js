...
export default {
  Query: {
    ...
    satistics: async (parent, { cursor, walletId }, { models, me }) => {
      ...
      // (with explicit ownership) so the per-row SQL filter is a simple
      // protocolId IN (...) lookup instead of a correlated triple-join.
      ...
      // Receive-side ownership for the activity branch. A row qualifies only
      // when the current user appears as the payee in one of three tables.
      // Extracting this fragment keeps the user-scoped predicates obvious at
      // the call site so they are harder to break in future refactors.
