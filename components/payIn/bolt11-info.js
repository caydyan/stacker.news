...
import { formatSats, msatsToSats, satsToMsats } from '@/lib/format'
...
export default function Bolt11Info ({
  ...
  const details = bolt11Details({ bolt11, hash, preimage, description, msats, expiresAt, confirmedAt, nostr, nostrNote, lud18Data, comment })
  ...
  return (
    <div className={styles.details}>
      ...
      {(details?.chips?.length > 0 || hasDetailPills) && (
...
export function toBolt11InfoProps ({
  bolt11,
  hash,
  preimage,
  confirmedPreimage,
...
function bolt11Details ({ bolt11, hash, preimage, description, msats, expiresAt, confirmedAt, nostr, nostrNote, lud18Data, comment }) {
  ...
  const paidChip = paidAt
    ? { key: 'paid', label: `paid ${timeSince(paidAt)}` }
  ...
  const chips = [
    ...
    paidChip,
    !paidChip && invoiceExpiresAt && { key: 'expires-relative', label: invoiceRelativeExpiryLabel(invoiceExpiresAt), tone: invoiceExpiresAt <= new Date() ? 'danger' : undefined },
...
function msatsToDisplaySats (msats) {
  const [amount, unit] = formatSats(msatsToSats(msats)).split(' ')
  return { amount, unit }
}
