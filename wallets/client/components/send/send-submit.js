...
// Only send a comment when the provider offered one; lnAddrInvoiceUrl rejects a
// comment a provider didn't offer (e.g. one left over from a previous provider).
function lnAddrSubmitValues (values, { commentAllowed } = {}) {
  return {
    ...(commentAllowed ? { comment: values.comment } : {}),
    ...Object.fromEntries(SUPPORTED_PAYER_DATA_FIELDS.map(field => [field, values[field]]))
  }
}
