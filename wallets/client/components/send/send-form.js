...
import { DestinationType, isLnAddrPendingOrReady, parseDestination } from './destination'
...
// lives only here, in which parent you render — no child reads a mode flag.
...
function useSendFormController ({ submit }) {
...
function SendFormFields ({ controller, enforcesMaxFee }) {
  ...
  const lnAddrPendingOrReady = isLnAddrPendingOrReady(destination, lnAddrLookup)
  const feeControl = (
    <FeeControl
      enforcesMaxFee={enforcesMaxFee}
      destination={destination}
      lnAddrPendingOrReady={lnAddrPendingOrReady}
    />
  )
  ...
  return (
    <>
      <div className={classNames(styles.fields, styles.formResponsiveReset, 'd-flex flex-column')}>
        ...
        />
        {destination.type === DestinationType.LN_ADDR && lnAddrPendingOrReady && (
