...
import { DestinationType, isLnAddrPendingOrReady, isLnAddrReady, lnAddrStatus, parseDestination } from './destination'
...
export function sendFormSchema ({ spendableSats, enforcesMaxFee, lnAddrLookup }) {
  ...
  return lazy(values => {
    ...
    const showLnAddr = isLnAddrPendingOrReady(dest, lnAddrLookup)
  ...
  return string()
    ...
    .test('destination', function (value) {
      ...
      if (dest.type === DestinationType.LN_ADDR) {
        if (isLnAddrReady(dest, lnAddrLookup)) return true
        if (lnAddrStatus(dest, lnAddrLookup) === 'error') {
