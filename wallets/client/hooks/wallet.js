...
export function useWalletCapabilities (wallet) {
  ...
  return useMemo(() => ({
    ...
    canSend: support.send && Boolean(sendProtocol) && ![WalletStatus.ERROR, WalletStatus.DISABLED].includes(status.send),
    canReceive: support.receive && Boolean(receiveProtocol) && ![WalletStatus.ERROR, WalletStatus.DISABLED].includes(status.receive)
  }), [receiveProtocol, sendProtocol, status.receive, status.send, support.receive, support.send, wallet.protocols.length])
}
