...
- add required triggers: `wallet_to_jsonb` (declared as `CONSTRAINT TRIGGER ... DEFERRABLE INITIALLY DEFERRED` so it fires at transaction commit and sees the Vault rows that Prisma writes after the relation row) and, if send protocol, also `wallet_clear_vault`
- run `npx prisma migrate dev`
  ...
  Because `wallet_to_jsonb` fires at commit, code that needs the materialized
  `WalletProtocol.config` should read the wallet after the write transaction
  commits. `saveWalletProtocols` follows this pattern: it writes inside a
  transaction, returns the wallet id, then hydrates the response with a fresh
  post-commit read.
