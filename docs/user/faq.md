...
Some wallet connections separate spending from balance reads. For NWC, the send string needs `get_balance`. For LNC, the session needs `/lnrpc.Lightning/ChannelBalance`. For CLNRest, the rune needs `method=listfunds`. Send-only connections can still pay, but their balance will show as unavailable.
