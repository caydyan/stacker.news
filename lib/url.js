...
// Read a response body as JSON while enforcing a byte cap so a hostile or
// runaway server cannot exhaust memory through an unbounded JSON payload.
export async function readJsonBody (res, { maxBytes = 256 * 1024 } = {}) {
  if (!res.body || typeof res.body.getReader !== 'function') {
    const text = await res.text()
    if (Buffer.byteLength(text, 'utf8') > maxBytes) {
      throw new ResponseAssertError(res, { message: `response body exceeds ${maxBytes} bytes` })
    }
